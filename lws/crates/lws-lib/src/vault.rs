...
/// Resolve the vault path: use explicit path if provided, otherwise default (~/.lws).
...
pub fn wallets_dir(vault_path: Option<&Path>) -> Result<PathBuf, LwsLibError> {
...
pub fn list_encrypted_wallets(
    vault_path: Option<&Path>,
) -> Result<Vec<EncryptedWallet>, LwsLibError> {
    let dir = wallets_dir(vault_path)?;
...
pub fn load_wallet_by_name_or_id(
    name_or_id: &str,
    vault_path: Option<&Path>,
) -> Result<EncryptedWallet, LwsLibError> {
    ...
    match matches.len() {
        0 => Err(LwsLibError::WalletNotFound(name_or_id.to_string())),
        1 => Ok(matches[0].clone()),
        n => Err(LwsLibError::AmbiguousWallet {
...
pub fn delete_wallet_file(id: &str, vault_path: Option<&Path>) -> Result<(), LwsLibError> {
    let dir = wallets_dir(vault_path)?;
    let path = dir.join(format!("{id}.json"));
    if !path.exists() {
        return Err(LwsLibError::WalletNotFound(id.to_string()));
    }
    fs::remove_file(&path)?;
    Ok(())
}
